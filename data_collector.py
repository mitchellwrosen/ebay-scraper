...
class PhoneBINScraper(PhoneScraper):
  ...
  def UpdateAverageSale(self, repeat_every=1):
    while True:
      self.avg_sale = db_util.UpdateAverageSale(self.cursor, self.id)
      LOG('Updating average sale. Now %s' % self.avg_sale)
...
if __name__ == '__main__':
  ...
  with con:
    ...
    for phone in config.kPhones:
      ...
      bin_scraper = PhoneBINScraper(cursor, url_info, phone, repeat_every=5)
